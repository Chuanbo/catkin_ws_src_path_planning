...
// for msg  用于引用自定义消息，编译时自动生成，因此忽略warning
...
public:
  ...
  DynamicObstacleDetector() : nh_("~"), n_(), tf_listener_(tf_buffer_) {
    ...
    ROS_INFO_STREAM(
        ...
        << std::endl
        << "- input scan topic: " << input_scan_topic_ << std::endl
        << "- odom_frame: " << odom_frame_ << std::endl
        << "- cluster_max_distance_points: " << thres_point_dist_ << std::endl
        << "- cluster_min_points: " << thresh_min_points_ << std::endl
        << "- cluster_max_points: " << thresh_max_points_ << std::endl
        << "- min_vel_tracked: " << min_vel_tracked_ << std::endl
        << "- max_vel_tracked: " << max_vel_tracked_ << std::endl
        << "- max_tracked_distance: " << track_distance_ << std::endl
        << "- max_tracked_sec: " << track_timeout_ << std::endl
        << std::endl);
    ...
    obs_pub_ = nh_.advertise<visualization_msgs::MarkerArray>(
        "/dynamic_obstacles/static_markers", 0);
    ...
    points_pub_ = nh_.advertise<visualization_msgs::Marker>(
        "/dynamic_obstacles/points", 0);

    obstacles_pub_ = nh_.advertise<dynamic_obstacle_detector::DynamicObstacles>(
        "/dynamic_obstacles", 0);
  }
    ...
    for (unsigned int i = 0; i < msg->ranges.size(); i++) {
      ...
      if (!isinf(msg->ranges[i]) && !isnan(msg->ranges[i])) {
        ...
        try {
          ...
          ROS_WARN("Could NOT transform point to %s: %s", odom_frame_.c_str(),
                   ex.what());
          return;
        }
        p.x = psn.point.x;
  ...
  void trackMovingObstaclesKF(std::vector<Obstacle> &obs) {
    ...
    for (int i = 0; i < (int)tracked_obstacles_.size(); i++) {
      ...
      if (minDist < track_distance_) {
        ...
        tracked_obstacles_[i].predict(
            (ros::Time::now() - tracked_obstacles_[i].tStamp).toSec());
    ...
    for (int i = 0; i < (int)tracked_obstacles_.size(); i++) {
      ...
      if ((ros::Time::now() - tracked_obstacles_[i].tStamp).toSec() <
  ...
  //   projector_.transformLaserScanToPointCloud("odom", *msg, cloud,
  //   tf_buffer_);
